...
#define WHILE_TEXT_W TEXT_W(5.75f)
...
static vec2_t gettopsize(CodeBlock *block) {
    ...
    return (vec2_t){ WHILE_TEXT_W + condSize.x + PADD * 2, max(HEIGHT, condSize.y + PADD * 2) };
}
...
static vec2_t condpos() {
    return (vec2_t){ WHILE_TEXT_W + PADD, PADD };
}
...
static vec2_t draw(CodeBlock *block, vec2_t pos) {
    ...
    text("while", pos.x + PADD + TEXT_W(0.25f), pos.y + TEXT_YOFFSET(topSize.y));
...
static char *totext(CodeBlock *block) {
    ...
    decTab();
    size_t len = 14 + CURR_TABSIZE + strlen(cond) + strlen(seq);

    char *txt = malloc((len+1) * sizeof(char));
    char *tabs = getTabs();
    sprintf(txt, "while %s do\n%s\n%send", cond, seq, tabs);
